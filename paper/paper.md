...
It has applications as a chelating agent, water treatment additive, drug carrier, biodegradable pressure‐sensitive adhesive tape, wound‐healing agents and many others \cite{Zargar2015AApplications,Shamshina2019AdvancesReview,RaviKumar2000AApplications}. 
...
There are three known crystal allomorphs \cite{Jang2004PhysicochemicalResources}: $\alpha$, $\beta$ and $\gamma$ chitin.
The crystal structures of $\alpha$ and $\beta$ chitin (the most abundant allomorphs) are known \cite{Sikorski2009Revisit,Nishiyama2011Xray} but the structure of the rare $\gamma$ chitin alomorph is still under debate \cite{Jang2004PhysicochemicalResources,Kaya2017On-chitin,Ramirez-Wong2016Sustainable-solvent-inducedFilms}.
...
However, up to date, there are only a few works that deal with all-atomic MD simulations of chitin  \cite{Yu2017Flexibility,McDonnell2016MolecularFilms,Strelcova2016TheSimulations,Jin2013MechanicalStudy}.
...
In the case of cellulose, there are many atomistic simulation works, deriving the most diverse features of cellulose from the known crystal structure \cite{Malaspina2019MolecularNanocrystals}.
We think that one possible reason for this difference is the availability of a cellulose builder tool \cite{Gomes2012Cellulose-Builder:Cellulose} that allows an easy build up of atomistic configurations and structure/topology files that can be used for MD simulations.
...
In this work we present a chitin builder tool implemented as a VMD plugin, that produces files containing atomic coordinates and topology information of pure $\alpha$ and $\beta$ chitin crystals of arbitrary size from experimental chitin crystal unit cells determined from X-ray crystallography \cite{Sikorski2009Revisit,Nishiyama2011Xray}.
...
However, we will not consider $\gamma$ chitin since its structure is not well known and in fact it is sometimes considered as a mixture of $\alpha$ and $\beta$ chitin \cite{Jang2004PhysicochemicalResources,Kaya2017On-chitin,Ramirez-Wong2016Sustainable-solvent-inducedFilms}.
The crystal structures of $\alpha$ and $\beta$ chitin allomorphs are shown in Figure \ref{fgr:crystals}.
In the case of $\alpha$ chitin, we show in Figure \ref{fgr:crystals}a the primitive cell as determined by from X-ray diffraction in Ref. \cite{Sikorski2009Revisit}.
...
In the case of $\beta$ chitin, it is important to note that $\beta$ chitin is partially soluble in water and in fact interaction with water seems to modify the crystal structure \cite{Jang2004PhysicochemicalResources}.
For this reason, we consider its anhydrous form, as determined by X-ray diffraction in Ref. \cite{Nishiyama2011Xray}.
...
%$\alpha$ chitin is the most abundant allomorph and is usually found in the shell of different organisms. The organization of $\alpha$ chitin is such that chitin chains are oriented in an antiparallel motif \cite{Zargar2015AApplications} as illustrated in Figure\ref{fgr:crystals} bottom left panel. Due to this chain organization $\alpha$ chitin produce the most stable fibrils of all allomorphs and the associated suprastructures display high hardness and low solubility. On the other hand $\beta$ chitin chain organization displays a parallel motif \cite{Zargar2015AApplications} as illustrated in Figure\ref{fgr:crystals} bottom right panel. This crystal organization of $\beta$ chitin favor the penetration of water and produce structures that are usually more flexible than $\alpha$ chitin. $\gamma$ chitin is less ubiquitous and is sometime consider as a mixture of $\alpha$ and $\beta$ chitin with a structure of chains of two antiparallel chains followed by a parallel chain \cite{Kaya2017On-chitin,Ramirez-Wong2016Sustainable-solvent-inducedFilms,Jang2004PhysicochemicalResources}.

An important feature determined in experiments is the presence of deacetylated groups (chitosan groups) in the fibril structure. This degree of deacetylation in chitin is of approximately between $5$ to 15$\%$ \cite{Zargar2015AApplications} although there are species that produce chitin of high purity.  
...
%Due to its complex structure and attractive properties there are there several studies on new applications of chitin based materials \cite{Shamshina2019AdvancesReview}. Although, from the molecular point of view there are several open questions in relation to chitin structure and its interactions with other molecules. 
...
In order to generate this additional information, we employ the topology from the carbohydrates section of CHARMM36 force field \cite{Guvench2011CHARMMModeling}.
...
The program generates two main outputs: a coordinate file (PDB) containing the position of all the atoms in the generated structure and a topology file (PSF) containing all the bonds, angles and dihedrals according to CHARMM36 carbohydrate section \cite{Guvench2011CHARMMModeling}.
...
This two files, plus the included CHARMM36 parameters file (located in the /ForceField/ folder) allows the user to easily start a molecular dynamics simulation using the NAMD simulation program \cite{Phillips2005ScalableNAMD} that accompanies VMD.
Also, using VMD, the users can easily export the data from these two files (PDB and PSF) to other coordinates formats or convert the topology to the formats required by other programs such as GROMACS \cite{VanDerSpoel2005GROMACS:Free,Hess2008GROMACSSimulation} using the topotools plugin included in VMD. 
...
\section{Compatibility and distribution}
